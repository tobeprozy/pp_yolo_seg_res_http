...
import logging
from python.ppocr_system_opencv import create_task, get_result
import threading
...
def handle_eval_request():
    ...
    if 'bmodel' in request_data:
        bmodel = request_data['bmodel']
        res_name = "ppocr_system_results_b4_" + bmodel + ".json"
        while True:
            result_files = os.listdir('results')
            if res_name in result_files:  # 检查res_name是否在文件列表中
                cmd = "python3 tools/eval_icdar.py --result_json results/" + res_name
