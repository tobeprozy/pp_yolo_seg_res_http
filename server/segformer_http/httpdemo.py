...
import logging
from python.segformer_opencv import create_task, get_result
import threading
...
def handle_create_request():
    ...
    if 'dataset_pt' in request_data:
        video_pt = request_data['dataset_pt']
...
def handle_eval_request():
    ...
    if 'bmodel' in request_data:
        bmodel = request_data['bmodel']
        res_name = "segformer.b0.512x1024.city.160k_" + bmodel + "_1b.bmodel_cityscapes_opencv_python_result.json"
        while True:
            result_files = os.listdir('results')
            if res_name in result_files:  # 检查res_name是否在文件列表中
                cmd = "python3 tools/segformer_eval.py --result_json " + res_name
                output_bytes = subprocess.check_output(cmd, shell=True)
                output_str = output_bytes.decode('utf-8')  # 转换为字符串
                response_data = {'message': 'F-score eval', 'output': output_str}
                return response_data, 200
            else:
