...
class SegFormer(object):
    def __init__(self, args):
        ...
        self.input_scale = self.net.get_input_scale(self.graph_name, self.input_names[0])
    ...
    def postprocess(self,bmimg_list, result):
        palette=dp.get_palette(self.palette)
        output_bmimgs=[]
...
# creat save path
    ...
    if os.path.isdir(args.input): 
        ...
        for filepath in image_files:  
            ...
            # 解码
            cn += 1
            logging.info("{}, img_file: {}".format(cn, filename))
            # decode
            start_time = time.time()
            ...
            if len(img_list) == batch_size:
                ...
                for i, filename in enumerate(filename_list):
                    ...
                    _, encoded_image = cv2.imencode('.jpg', seg[i])
                    base64_image = base64.b64encode(encoded_image).decode('utf-8')
                    put_element(my_queue, [cn, base64_image])
    ...
    preprocess_time = segformer.preprocess_time / cn
    inference_time = segformer.inference_time / cn
    postprocess_time = segformer.postprocess_time / cn
...
def argsparser():
    ...
    parser.add_argument('--input', type=str, default='datasets/cityscapes', help='path of input, must be image directory')
...
def create_task(bmodel):
    print("===============")
    print("segformer-TEST")
    print("===============")
    args = argsparser()
    if bmodel == "int8":
        args.bmodel = 'models/BM1684/segformer.b0.512x1024.city.160k_int8_1b.bmodel'
    main(args)
